...
class SimulatedDSPlayer:
    ...
    def build_packet(self, msg_type, payload):
        ...
        # New format: version(1), msgType(1), playerID(1), seqNum(1) = 4 bytes header
        header = struct.pack('<BBBB', PROTOCOL_VERSION, msg_type,
                           self.player_id, self.sequence_number & 0xFF)
        self.sequence_number = (self.sequence_number + 1) & 0xFF  # Wrap at 255
        return header + payload

    def send_lobby_join(self):
        """Send MSG_LOBBY_JOIN"""
        # Payload is now 28 bytes (was 24)
        payload = struct.pack('<B27x', int(self.is_ready))
    ...
    def send_car_update(self):
        ...
        # carState: position(8), speed(4), angle512(4), lap(4), item(4), reserved(4) = 28 bytes
        payload = struct.pack('<iiiiii4x', pos_x_q16_8, pos_y_q16_8, speed_q16_8,
    ...
    def send_item_placed(self, item_type):
        ...
        # itemPlaced: itemType(4), position(8), angle512(4), speed(4), reserved(8) = 28 bytes
        payload = struct.pack('<iiiii8x', item_type, pos_x_q16_8, pos_y_q16_8,
    ...
    def receive_packets(self):
        ...
        try:
            ...
            if len(data) >= 4:  # Header is now 4 bytes (was 8)
                version, msg_type, player_id, seq_num = struct.unpack('<BBBB', data[:4])
                ...
                if msg_type in [MessageType.MSG_LOBBY_JOIN, MessageType.MSG_READY, MessageType.MSG_DISCONNECT]:
                    print(f"📥 Received {msg_type_name} from Player {player_id + 1} (seq={seq_num})")
...
Examples:
  ...
  python simulate_ds_player.py --player-id 3 --port 8888
    ...
    # Use the target IP from args (defaults to 192.168.0.100)
